...
class vit_bb_ss(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        if self.config.name_model == 'xr14':
        ...
        elif self.config.name_model == 'vit_bb':
            self.conv3_ss_f = ConvBlock(channel=[
        ...
        self.gru = nn.GRUCell(input_size=8, hidden_size=config.n_decoder[4][0])
        ...
        self.controller = nn.Sequential(
            ...
            nn.Linear(config.n_decoder[3][-1], 2),  # str dan thrt
            nn.ReLU()
        )
        self.patch_size = 16
        self.num_layers = 12
    ...
    def forward(self, rgbs, pt_cloud_xs, pt_cloud_zs, rp1, rp2, velo_in):
        ...
        # Expand the class token to the full batch
            ...
            if self.config.name_model == 'vit_bb':
