...
def get_wprp_frame(config, wp, rp1, rp2, lr_velo, str, thrt, bearing, seq, latlon):
    meta_output = dict()
    frame_h, frame_w = int(
        config.res_resize[0]), int(config.res_resize[1])  # HxW
    ...
    for i in range(0, config.pred_len):
        x_frame = np.clip(
            int((wp[i][0]+area)*(frame_w-1)/(2*area)), 0, frame_w-1)  # constrain
        y_frame = np.clip(
            int((wp[i][1]*(1-frame_h)/area) + (frame_h-1)), 0, frame_h-1)  # constrain
        wp_frame.append(np.array([x_frame, y_frame]))

    # proses juga untuk next route
    rp1_x_frame = np.clip(
        int((rp1[0]+area)*(frame_w-1)/(2*area)), 0, frame_w-1)  # constrain
    rp1_y_frame = np.clip(
        int((rp1[1]*(1-frame_h)/area) + (frame_h-1)), 0, frame_h-1)  # constrain
    rp1_frame = np.array([rp1_x_frame, rp1_y_frame])

    rp2_x_frame = np.clip(
        int((rp2[0]+area)*(frame_w-1)/(2*area)), 0, frame_w-1)  # constrain
    rp2_y_frame = np.clip(
        int((rp2[1]*(1-frame_h)/area) + (frame_h-1)), 0, frame_h-1)  # constrain
...
def save_out(config, seg, sdc, save_dir, condition, route, filename, metax):
    ...
    hhh = int(config.res_resize[0])
    imgx3 = imgx2.copy()
    # plot rp
    imgx3 = cv2.circle(imgx3, (metax['rp1_frame'][0], metax['rp1_frame'][1]), radius=3, color=(
        255, 255, 255), thickness=1)
    imgx3 = cv2.circle(imgx3, (metax['rp2_frame'][0], metax['rp2_frame'][1]), radius=3, color=(
        255, 255, 255), thickness=1)
    for k in range(config.pred_len):  # plot wp
        imgx3 = cv2.circle(imgx3, (metax['wp_frame'][k][0], metax['wp_frame'][k][1]), radius=1, color=(
            255, 255, 255), thickness=-1)
    cv2.putText(imgx3, "Sequence: "+str(metax['seq']), (10, hhh-80),
                cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "Steering: "+str(np.round(metax['str'], decimals=5)),
                (10, hhh-70), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "Throttle: "+str(np.round(metax['thrt'], decimals=5)),
                (10, hhh-60), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "LR speed: "+str(np.round(metax['lr_velo'], decimals=5)), (
        10, hhh-50), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "RP 1: "+str(np.round(metax['rp1'], decimals=5)),
                (10, hhh-40), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "RP 2: "+str(np.round(metax['rp2'], decimals=5)),
                (10, hhh-30), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "Bearing: "+str(np.round(metax['bearing'], decimals=5)),
                (10, hhh-20), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)
    cv2.putText(imgx3, "Lat-Lon: "+str(np.round(metax['latlon'], decimals=5)),
                (10, hhh-10), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255), 1, 2)

    # cetak predicted segmentation
    cv2.imwrite(save_dir_sdc_wprp+filename, imgx3)
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            for i in range(0, config.seq_len):  # append data untuk input sequence
                ...
                pt_cloud_xs.append(data['pt_cloud_xs'][i].to(
                    device, dtype=torch.float))
                pt_cloud_zs.append(data['pt_cloud_zs'][i].to(
                    device, dtype=torch.float))
            rp1 = torch.stack(data['rp1'], dim=1).to(device, dtype=torch.float)
            rp2 = torch.stack(data['rp2'], dim=1).to(device, dtype=torch.float)
            gt_velocity = torch.stack(data['lr_velo'], dim=1).to(
                device, dtype=torch.float)
            gt_waypoints = [torch.stack(data['waypoints'][j], dim=1).to(
                device, dtype=torch.float) for j in range(0, config.pred_len)]
            ...
            pred_segs, pred_wp, psteering, pthrottle, sdcs = model(
                rgbs, pt_cloud_xs, pt_cloud_zs, rp1, rp2, gt_velocity)  # , seg_fronts[-1])
            # print(len(sdcs), len(sdcs[-1]), len(sdcs[-1][-1]))
            # show_sdc(sdcs[-1])
            steering, throttle, meta_pred = model.mlp_pid_control(
                pred_wp, lr_velo[-1], psteering, pthrottle)
            ...
            total_metric = (1-metric_seg.item()) + \
                metric_wp.item() + metric_str + metric_thr
            ...
            # save outputnya
            meta_output = get_wprp_frame(config, pred_wp[0].cpu().detach().numpy(), rp1[0].cpu().detach().numpy(), rp2[0].cpu().detach().numpy(), [data['lr_velo'][0].item(
            ), data['lr_velo'][1].item()], psteering.item(), pthrottle.item(), data['bearing_robot'].item(), data['filename'][-1][-8:-4], [data['lat_robot'].item(), data['lon_robot'].item()])
            save_out(config, pred_segs[-1], sdcs[-1], save_dir, data['condition'][-1], data['route'][-1],
                     data['filename'][-1], meta_output)  # ambil pred_segs dan sdcs terakhir saja untuk diproses

            save_dir_meta = save_dir+'outputs/' + \
                data['condition'][-1]+'/'+data['route'][-1]+'/meta/'
            os.makedirs(save_dir_meta, exist_ok=True)
...
model = swin(config, device).to(device, dtype=torch.float)
model.load_state_dict(torch.load(
    os.path.join(config.logdir, 'best_model.pth')))

# BUAT DATA BATCH
test_set = WHILL_Data(data_root=config.test_dir,
                      conditions=config.test_conditions, config=config)
dataloader_test = DataLoader(
    test_set, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)  # BS selalu 1
