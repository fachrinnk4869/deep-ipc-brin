...
def check_gt_seg(config, gt_seg):
    ...
    cv2.imwrite(config.logdir+"/check_gt_seg.png",
                imgx)  # cetak gt segmentation
...
def train(data_loader, model, config, writer, cur_epoch, device, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        total_loss = params_lw[0]*loss_seg + params_lw[1] * \
            loss_wp + params_lw[2]*loss_str + params_lw[3]*loss_thr
        ...
        if batch_ke == 0:  # batch pertama, hitung loss awal
            ...
            if config.MGN:
                ...
                # backpro, hitung gradient, retain graph karena graphnya masih dipakai perhitungan
                ...
                # dan rata2
                loss_hat_avg = (loss_seg_hat + loss_wp_hat +
                                loss_str_hat + loss_thr_hat) / len(config.loss_weights)
                ...
                Lgrad = F.l1_loss(G0, C0) + F.l1_loss(G1, C1) + \
                    F.l1_loss(G2, C2) + F.l1_loss(G3, C3)
...
def main():
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optima, mode='min', factor=0.5, patience=4, min_lr=1e-6)

    # BUAT DATA BATCH
    train_set = WHILL_Data(data_root=config.train_dir,
                           conditions=config.train_conditions, config=config)
    val_set = WHILL_Data(data_root=config.val_dir,
                         conditions=config.val_conditions, config=config)
    ...
    while True:
        ...
        print('| t_total_l: %.4f | t_ss_l: %.4f | t_wp_l: %.4f | t_str_l: %.4f | t_thr_l: %.4f |' % (
            train_log['t_total_l'], train_log['t_ss_l'], train_log['t_wp_l'], train_log['t_str_l'], train_log['t_thr_l']))
        print('| v_total_l: %.4f | v_ss_l: %.4f | v_wp_l: %.4f | v_str_l: %.4f | v_thr_l: %.4f |' % (
            val_log['v_total_l'], val_log['v_ss_l'], val_log['v_wp_l'], val_log['v_str_l'], val_log['v_thr_l']))
