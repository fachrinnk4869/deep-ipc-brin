...
class WHILL_Data(Dataset):
    ...
    def __init__(self, data_root, conditions, config):
        ...
        for condition in conditions:
            ...
            preload_file = os.path.join(sub_root, 'skgeswin_seq'+str(self.seq_len)+'_pred'+str(
            ...
            if not os.path.exists(preload_file):
                ...
                for route in routes:
                    ...
                    with open(route_dir+"/"+route+"_routepoint_list.yml", 'r') as rp_listx:
                        # with open(route_dir+"/gmaps"+route[-2:]+"_routepoint_list.yaml", 'r') as rp_listx:
                        rp_list = yaml.full_load(rp_listx)
                        # assign end point sebagai route terakhir
                        rp_list['route_point']['latitude'].append(
                            rp_list['last_point']['latitude'])
                        rp_list['route_point']['longitude'].append(
                            rp_list['last_point']['longitude'])

                    # list dan sort file, slah satu saja
                    files = os.listdir(route_dir+"/camera/front/rgb/")
                    ...
                    if self.config.n_buffer != 0:
                        ...
                        for _ in range(0, self.config.n_buffer*self.data_rate-1):
                            sin_angle_buff.append(
                                np.sin(np.radians(first_meta['global_orientation_rpy'][2])))

                    # kurangi sesuai dengan jumlah sequence dan wp yang akan diprediksi
                    for i in range(0, len(files)-(self.seq_len-1)-(self.pred_len*self.data_rate)):
                        ...
                        for j in range(0, self.seq_len):
                            filename = files[i+j]
                            rgbs.append(
                                route_dir+"/camera/front/rgb/"+filename)
                            segs.append(
                                route_dir+"/camera/front/seg/map/"+filename)
                            pt_clouds.append(
                                route_dir+"/camera/front/depth/cld/"+filename[:-3]+"npy")
                        ...
                        loc_headings.append(np.radians(
                            meta_current['global_orientation_rpy'][2]))
                        preload_lon.append(
                            meta_current['global_position_latlon'][1])
                        preload_lat.append(
                            meta_current['global_position_latlon'][0])

                        # apply MAF ke bearing
                        angle_deg = meta_current['global_orientation_rpy'][2]
                        sin_angle_buff.append(np.sin(np.radians(angle_deg)))
                        sin_angle_buff_mean = np.array(sin_angle_buff).mean()
                        # cek kuadran
                        if 0 < angle_deg <= 90:  # Q1
                            angle_deg_maf = np.degrees(
                                np.arcsin(sin_angle_buff_mean))
                        elif 90 < angle_deg <= 180:  # Q2
                            angle_deg_maf = 180 - \
                                np.degrees(np.arcsin(sin_angle_buff_mean))
                        ...
                        preload_steering.append(0)
                        preload_throttle.append(0)

                        # kecepatan LR dibuat positif semua
                        # preload_velocity_l.append(
                        #     np.abs(meta_current['whill_LR_wheel_angular_velo'][0]))
                        # # kecepatan LR dibuat positif semua
                        # preload_velocity_r.append(
                        #     np.abs(meta_current['whill_LR_wheel_angular_velo'][1]))
                        preload_velocity_l.append(
                            np.abs(0))
                        # kecepatan LR dibuat positif semua
                        preload_velocity_r.append(
                        ...
                        for r in range(2):  # ada 2 route point
                            ...
                            # 111320 #Y
                            dLat_m = (
                                next_lat-meta_current['global_position_latlon'][0]) * 40008000 / 360
                            dLon_m = (next_lon-meta_current['global_position_latlon'][1]) * 40075000 * np.cos(
                                np.radians(meta_current['global_position_latlon'])) / 360  # X

                            # jika jarak euclidian rp1 <= jarak min, hapus route dan loncat ke next route
                            if r == 0 and np.all(np.sqrt(dLat_m**2 + dLon_m**2) <= self.rp1_close) and not about_to_finish:
                                # jika jumlah route list masih > 2
                        ...
                        for k in range(1, self.pred_len+1):
                            # future seconds, makanya dikali data rate
                            filenamef = files[(
                            ...
                            loc_ys.append(
                                meta_future['local_position_xyz'][1])
                            loc_headings.append(np.radians(
                                meta_future['global_orientation_rpy'][2]))
            ...
            print("Preloading " + str(len(preload_dict.item()
                  ['rgb'])) + " sequences from " + preload_file)
    ...
    def __getitem__(self, index):
        ...
        dLon2_m = (self.rp2_lon[index]-lon_robot) * \
            40075000 * np.cos(np.radians(lat_robot)) / 360  # X
        ...
        data['lr_velo'] = tuple(
            np.array([self.velocity_l[index], self.velocity_r[index]]))
...
def crop_matrix(image, resize=1, D3=True, crop=[512, 1024]):
    ...
    upper_left_yx = [int((image.shape[0]/2) - (crop[0]/2)),
                     int((image.shape[1]/2) - (crop[1]/2))]
    if D3:  # buat matrix 3d
        cropped_im = image[upper_left_yx[0]:upper_left_yx[0] +
                           crop[0], upper_left_yx[1]:upper_left_yx[1]+crop[1], :]
    else:  # buat matrix 2d
        cropped_im = image[upper_left_yx[0]:upper_left_yx[0] +
                           crop[0], upper_left_yx[1]:upper_left_yx[1]+crop[1]]

    # resize image
    WH_resized = (int(cropped_im.shape[1]/resize),
                  int(cropped_im.shape[0]/resize))
    resized_image = cv2.resize(
        cropped_im, WH_resized, interpolation=cv2.INTER_NEAREST)
